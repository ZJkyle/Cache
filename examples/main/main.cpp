...
static void write_logfile(const llama_context *ctx, const gpt_params &params,
  ...
  dump_non_result_info_yaml(logfile, params, ctx, timestamp, input_tokens,
                            model_desc);
...
static void sigint_handler(int signo) {
  if (signo == SIGINT) {
    if (!is_interacting && g_params->interactive) {
      ...
      write_logfile(*g_ctx, *g_params, *g_model, *g_input_tokens,
                    g_output_ss->str(), *g_output_tokens);
      _exit(130);
    }
  }
}
...
static void llama_log_callback_logTee(ggml_log_level level, const char *text,
                                      void *user_data) {
  (void)level;
  (void)user_data;
  LOG_TEE("%s", text);
...
int main(int argc, char **argv) {
  ...
  if (n_ctx > n_ctx_train) {
    LOG_TEE("%s: warning: model was trained on only %d context tokens (%d "
            "specified)\n",
  ...
  if (ctx_guidance) {
    ...
    guidance_inp =
        ::llama_tokenize(ctx_guidance, sparams.cfg_negative_prompt, true, true);
    LOG("guidance_inp tokenized: %s\n",
        LOG_TOKENS_TOSTR_PRETTY(ctx_guidance, guidance_inp).c_str());

    std::vector<llama_token> original_inp =
        ::llama_tokenize(ctx, params.prompt, true, true);
  ...
  if (!session_tokens.empty()) {
    ...
    // remove any "future" tokens that we might have inherited from the previous
    // session
  ...
  if (ga_n != 1) {
    ...
    // multiple of grp_attn_w");    // NOLINT GGML_ASSERT(n_ctx >= n_ctx_train *
    // ga_n && "n_ctx must be at least n_ctx_train * grp_attn_n"); // NOLINT
  ...
  while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
    ...
    if (!embd.empty()) {
      ...
      if (ga_n == 1) {
        ...
        if (n_past + (int)embd.size() + std::max<int>(0, guidance_offset) >=
          ...
          LOG("context full, swapping: n_past = %d, n_left = %d, n_ctx = %d, "
    ...
    if ((int)embd_inp.size() <= n_consumed && !is_interacting) {
      ...
      while ((int)embd_inp.size() > n_consumed) {
        ...
        // push the prompt in the sampling context in order to apply repetition
        // penalties later for the prompt, we don't apply grammar rules
    ...
    if (input_echo && display) {
      for (auto id : embd) {
        const std::string token_str =
            llama_token_to_piece(ctx, id, !params.conversation);
    ...
    if ((int)embd_inp.size() <= n_consumed) {
        ...
        if (params.interactive) {
          if (!params.antiprompt.empty()) {
            ...
            const auto first_antiprompt =
                ::llama_tokenize(ctx, params.antiprompt.front(), false, true);
      ...
      if (n_past > 0 && is_interacting) {
        ...
        if (buffer.length() > 1) {
          ...
          const auto line_pfx =
              ::llama_tokenize(ctx, params.input_prefix, false, true);
          const auto line_inp =
              ::llama_tokenize(ctx, buffer, false, params.interactive_specials);
          const auto line_sfx =
    ...
    // to user input when reached. We skip this logic when n_predict == -1
    // (infinite) or -2 (stop at context size).
  ...
  write_logfile(ctx, params, model, input_tokens, output_ss.str(),
                output_tokens);
