...
static void write_logfile(const llama_context *ctx, const gpt_params &params,
  ...
  if (params.hellaswag) {
    fprintf(stderr,
            "%s: warning: logging results is not implemented for HellaSwag. No "
  ...
  if (!success) {
    fprintf(stderr,
            "%s: warning: failed to create logdir %s, cannot write logfile\n",
  ...
  dump_non_result_info_yaml(logfile, params, ctx, timestamp, results.tokens,
                            model_desc);
...
static results_log_softmax log_softmax(int n_vocab, const float *logits,
                                       int tok) {
  float max_logit = logits[0];
  ...
  return {logits[tok] - max_logit - log(sum_exp), logits[tok],
...
static double log_softmax(int n_vocab, const float *logits, uint16_t *log_prob,
  ...
  if (scale) {
    ...
    for (int i = 0; i < n_vocab; ++i) {
      log_prob[i] = logits[i] > min_logit
                        ? nearest_int(inv_scale * (logits[i] - min_logit))
                        : 0;
    }
  } else {
...
static void process_logits(int n_vocab, const float *logits, const int *tokens,
                           int n_token, std::vector<std::thread> &workers,
                           double &nll, double &nll2, float *logit_history,
                           float *prob_history) {
  std::mutex mutex;
  int counter = 0;
  auto compute = [&mutex, &counter, &nll, &nll2, logit_history, prob_history,
    ...
    while (true) {
      ...
      lock.unlock();
      const results_log_softmax results =
          log_softmax(n_vocab, logits + i * n_vocab, tokens[i + 1]);
...
static void process_logits(std::ostream &out, int n_vocab, const float *logits,
                           const int *tokens, int n_token,
                           std::vector<std::thread> &workers,
                           std::vector<uint16_t> &log_probs, double &nll,
                           double &nll2) {
  std::mutex mutex;
  const int nv = 2 * ((n_vocab + 1) / 2) + 4;
  int counter = 0;
  auto compute = [&mutex, &counter, &log_probs, &nll, &nll2, n_vocab, logits,
                  tokens, n_token, nv]() {
    double local_nll = 0;
    double local_nll2 = 0;
    while (true) {
      ...
      lock.unlock();
      const double v = log_softmax(n_vocab, logits + i * n_vocab,
                                   log_probs.data() + i * nv, tokens[i + 1]);
      local_nll += v;
      local_nll2 += v * v;
    }
  };
...
static std::pair<double, float> log_softmax(int n_vocab, const float *logits,
...
static void process_logits(int n_vocab, const float *logits, const int *tokens,
                           int n_token, std::vector<std::thread> &workers,
                           const std::vector<uint16_t> &base_log_probs,
                           kl_divergence_result &kld, float *kld_values,
                           float *p_diff_values) {
  std::mutex mutex;
  const int nv = 2 * ((n_vocab + 1) / 2) + 4;
  int counter = 0;
  auto compute = [&mutex, &counter, &base_log_probs, &kld, n_vocab, logits,
                  tokens, n_token, nv, kld_values, p_diff_values]() {
    ...
    while (true) {
      ...
      lock.unlock();
      std::pair<double, float> v =
          log_softmax(n_vocab, logits + i * n_vocab,
                      base_log_probs.data() + i * nv, tokens[i + 1], local_kld);
...
static results_perplexity perplexity_v2(llama_context *ctx,
  ...
  if (int(tokens.size()) < 2 * n_ctx) {
    fprintf(stderr,
            "%s: you need at least %d tokens to evaluate perplexity with a "
            "context of %d\n",
            __func__, 2 * n_ctx, n_ctx);
    fprintf(stderr,
            "%s: the data file you provided tokenizes to only %zu tokens\n",
  ...
  fprintf(stderr, "%s: have %zu tokens. Calculation chunk = %d\n", __func__,
          tokens.size(), calc_chunk);

  if (int(tokens.size()) <= calc_chunk) {
    fprintf(stderr,
            "%s: there are only %zu tokens, this is not enough for a context "
            "size of %d and stride %d\n",
            __func__, tokens.size(), n_ctx, params.ppl_stride);
    return {tokens, -1, logit_history, prob_history};
  }

  const int n_chunk_max =
      (tokens.size() - calc_chunk + params.ppl_stride - 1) / params.ppl_stride;

  const int n_chunk = params.n_chunks < 0
                          ? n_chunk_max
                          : std::min(params.n_chunks, n_chunk_max);
  ...
  for (int i = 0; i < n_chunk; ++i) {
    ...
    for (int j = 0; j < num_batches; ++j) {
      ...
      // fprintf(stderr, "    Batch %d: starts at %d, size is %d, n_past is
      // %d\n",j,batch_start,batch_size,j * n_batch);
      //  TODO: use llama_batch.logits instead of relying on logits_all == true
      if (llama_decode(ctx, llama_batch_get_one(tokens.data() + batch_start,
                                                batch_size, j * n_batch, 0))) {
      ...
      logits.insert(logits.end(), batch_logits,
                    batch_logits + batch_size * n_vocab);
    ...
    // params.ppl_stride + start, params.n_ctx + start);
...
static results_perplexity
perplexity(llama_context *ctx, const gpt_params &params, const int32_t n_ctx) {
  if (params.ppl_stride > 0) {
  ...
  fprintf(stderr, "%s: tokenization took %g ms\n", __func__,
          1e-3 *
              std::chrono::duration_cast<std::chrono::microseconds>(tim2 - tim1)
                  .count());
  ...
  if (!params.logits_file.empty()) {
    ...
    logits_stream.write((const char *)tokens.data(),
                        n_chunk * n_ctx * sizeof(tokens[0]));
    const int nv = 2 * ((n_vocab + 1) / 2) + 4;
  ...
  // previous tokens here, so the logits returned for each token are an accurate
  // representation of what the model would have predicted at that point.
  //
  ...
  for (int i = 0; i < n_chunk; i += n_seq) {
    ...
    for (int j = 0; j < num_batches; ++j) {
      ...
      if (num_batches > 1 && n_outputs > 0) {
        const auto *batch_logits = llama_get_logits(ctx);
        logits.insert(logits.end(), batch_logits,
                      batch_logits + n_outputs * n_vocab);
      }
    }
    ...
    for (int seq = 0; seq < n_seq_batch; seq++) {
      const float *all_logits =
          num_batches > 1 ? logits.data()
                          : llama_get_logits_ith(ctx, seq * n_ctx + first);

      llama_token *tokens_data = tokens.data() + start + seq * n_ctx + first;
      if (!params.logits_file.empty()) {
        process_logits(logits_stream, n_vocab, all_logits, tokens_data,
                       n_ctx - 1 - first, workers, log_probs, nll, nll2);
      } else {
        process_logits(n_vocab, all_logits, tokens_data, n_ctx - 1 - first,
                       workers, nll, nll2,
...
static bool decode_helper(llama_context *ctx, llama_batch &batch,
                          std::vector<float> &batch_logits, int32_t n_batch,
                          int32_t n_vocab) {
  int prev_outputs = 0;
  for (int32_t i = 0; i < (int32_t)batch.n_tokens; i += n_batch) {
    ...
    memcpy(batch_logits.data() + prev_outputs * n_vocab, llama_get_logits(ctx),
...
static void
compute_logprobs(const float *batch_logits, int n_vocab,
                 std::vector<std::thread> &workers,
                 const std::vector<std::pair<size_t, llama_token>> &eval_pairs,
  ...
  size_t max_threads = std::min(
      (eval_pairs.size() + K_TOKEN_CHUNK - 1) / K_TOKEN_CHUNK, workers.size());

  std::atomic<int> counter(0);
  auto compute = [&counter, &eval_pairs, &eval_results, batch_logits,
                  n_vocab]() {
    float local_logprobs[K_TOKEN_CHUNK];
    while (true) {
      size_t first =
          counter.fetch_add(K_TOKEN_CHUNK, std::memory_order_relaxed);
      ...
      for (size_t i = first; i < last; ++i) {
        ...
        local_logprobs[i - first] =
            logits[eval_pairs[i].second] - max_logit - std::log(sum_p);
      }
      std::memcpy(eval_results.data() + first, local_logprobs,
...
static void hellaswag_score(llama_context *ctx, const gpt_params &params) {
  ...
  // https://github.com/EleutherAI/lm-evaluation-harness/blob/df3da98c5405deafd519c2ddca52bb7c3fe36bef/lm_eval/tasks/hellaswag.py#L62-L68
  //
  // All 10042 tasks should be extracted to keep the results standardized like
  // other implementations.
  //
  // Datafile layout:
  // ['??'] denotes json fields
  // 6 lines per task:
  // ['activity_label'] + ": " +['ctx']  - The first part of the query, the
  // context
  ...
  const bool is_spm =
      llama_vocab_type(llama_get_model(ctx)) == LLAMA_VOCAB_TYPE_SPM;
  ...
  // The random seed should not impact the final result if the computation is
  // done over enough tasks, so kept hardcoded for now
  std::mt19937 rng(1);
  ...
  struct hs_data_t {
    ...
    size_t common_prefix;   // max number of initial tokens that are the same in
                            // all sentences
  ...
  fprintf(stderr, "%s : selecting %zu %s tasks.\n", __func__, hs_task_count,
          (randomize_tasks ? "randomized" : "the first"));
  ...
  for (size_t i = 0; i < hs_task_count; i++) {
    ...
    for (size_t j = 0; j < 4; j++) {
      ...
      hs_cur.seq_tokens[j] =
          ::llama_tokenize(ctx, hs_cur.context + " " + hs_cur.ending[j], true);
    }
    ...
    hs_cur.required_tokens =
        hs_cur.common_prefix + hs_cur.seq_tokens[0].size() -
        hs_cur.common_prefix + hs_cur.seq_tokens[1].size() -
        hs_cur.common_prefix + hs_cur.seq_tokens[2].size() -
        hs_cur.common_prefix + hs_cur.seq_tokens[3].size() -
        hs_cur.common_prefix;

    // GGML_ASSERT(hs_cur.common_prefix >= ::llama_tokenize(ctx, hs_cur.context,
    // true).size());

    // Delete the selected random example from the prompt
    if (randomize_tasks) {
      prompt_lines.erase(std::next(prompt_lines.begin(), idx * 6),
                         std::next(prompt_lines.begin(), idx * 6 + 6));
    }
  }
  ...
  const int max_seq =
      std::min(4 * max_tasks_per_batch, (int)llama_n_seq_max(ctx));
  ...
  for (size_t i0 = 0; i0 < hs_task_count; i0++) {
    ...
    size_t i1 = i0;
    size_t i_logits = 0; // this tells us how many logits were needed before
    ...
    // we extract logits only from the last common token and from all ending
    ...
    while (n_cur + (int)hs_data[i1].required_tokens <= n_ctx) {
      ...
      for (size_t i = 0; i < hs_cur.common_prefix; ++i) {
        llama_batch_add(batch, hs_cur.seq_tokens[0][i], i,
                        {s0 + 0, s0 + 1, s0 + 2, s0 + 3}, false);
      }
      batch.logits[batch.n_tokens - 1] =
          true; // we need logits for the last token of the common prefix
      n_logits += 1;
      ...
      for (int s = 0; s < 4; ++s) {
        ...
        for (size_t i = hs_cur.common_prefix; i < seq_tokens_size; ++i) {
          ...
          llama_batch_add(batch, hs_cur.seq_tokens[s][i], i, {s0 + s},
                          needs_logits);
    ...
    for (size_t i = i0; i < i1; ++i) {
      auto &hs_cur = hs_data[i];
      size_t li = 1; // skip the last logit of the common prefix (computed
                     // separately below)
      for (int s = 0; s < 4; ++s) {
        for (size_t j = hs_cur.common_prefix;
             j < hs_cur.seq_tokens[s].size() - 1; j++) {
          eval_pairs.emplace_back(hs_cur.i_logits + li++,
                                  hs_cur.seq_tokens[s][j + 1]);
        }
      }
    }
    // Then we do the actual calculation
    compute_logprobs(batch_logits.data(), n_vocab, workers, eval_pairs,
                     eval_results);
    ...
    for (size_t i = i0; i < i1; ++i) {
      ...
      std::memcpy(tok_logits.data(),
                  batch_logits.data() + n_vocab * hs_cur.i_logits,
      ...
      for (int s = 0; s < 4; ++s) {
        ...
        hs_cur.ending_logprob[s] =
            std::log(first_probs[hs_cur.seq_tokens[s][hs_cur.common_prefix]]);
        for (size_t j = hs_cur.common_prefix;
             j < hs_cur.seq_tokens[s].size() - 1; j++) {
...
static std::vector<winogrande_entry>
load_winogrande_from_csv(const std::string &prompt) {
  ...
  while (true) {
    ...
    auto sentence =
        line[comma_pos[0] + 1] == '"'
            ? line.substr(comma_pos[0] + 2, comma_pos[1] - comma_pos[0] - 3)
            : line.substr(comma_pos[0] + 1, comma_pos[1] - comma_pos[0] - 1);
    auto choice1 =
        line.substr(comma_pos[1] + 1, comma_pos[2] - comma_pos[1] - 1);
    auto choice2 =
        line.substr(comma_pos[2] + 1, comma_pos[3] - comma_pos[2] - 1);
 ...
 * Uses a CSV containing task index, dentence, choice 1, choice 2, answer (1 or
 * 2) You can get one such dataset from e.g.
 * https://huggingface.co/datasets/ikawrakow/winogrande-eval-for-llama.cpp As an
 * example, the 1st row in the above dataset is
 *
 *    0,Sarah was a much better surgeon than Maria so _ always got the easier
 * cases.,Sarah,Maria,2
 *
 */
static void winogrande_score(llama_context *ctx, const gpt_params &params) {
  ...
  for (auto &task : data) {
    task.seq_tokens[0] =
        ::llama_tokenize(ctx, task.first + task.choices[0] + task.second, true);
    task.seq_tokens[1] =
        ::llama_tokenize(ctx, task.first + task.choices[1] + task.second, true);
    ...
    task.required_tokens = task.common_prefix + task.seq_tokens[0].size() -
                           task.common_prefix + task.seq_tokens[1].size() -
                           task.common_prefix;

    task.n_base1 =
        ::llama_tokenize(ctx, task.first + task.choices[0], true).size();
    task.n_base2 =
        ::llama_tokenize(ctx, task.first + task.choices[1], true).size();
  }
  ...
  const int max_seq =
      std::min(2 * max_tasks_per_batch, (int)llama_n_seq_max(ctx));
  ...
  for (size_t i0 = 0; i0 < data.size(); i0++) {
    ...
    while (n_cur + (int)data[i1].required_tokens <= n_ctx) {
      ...
      for (size_t i = 0; i < data[i1].common_prefix; ++i) {
        llama_batch_add(batch, data[i1].seq_tokens[0][i], i, {s0 + 0, s0 + 1},
                        false);
      }
      ...
      for (int s = 0; s < 2; ++s) {
        // TODO: end before the last token, no need to predict past the end of
        // the sequences
        for (size_t i = data[i1].common_prefix;
             i < data[i1].seq_tokens[s].size(); ++i) {
    ...
    for (size_t i = i0; i < i1; ++i) {
      ...
      for (size_t j = n_base1 - 1; j < task.seq_tokens[0].size() - 1 - last_1st;
           ++j) {
        eval_pairs.emplace_back(task.i_logits + li++,
                                task.seq_tokens[0][j + 1]);
      }
      const auto &n_base2 = skip_choice ? task.n_base2 : task.common_prefix;
      const int last_2nd = task.seq_tokens[1].size() - n_base2 > 1 ? 1 : 0;
      // FIXME: this uses the wrong first logits when not skipping the choice
      // word
      li = task.seq_tokens[0].size() - task.common_prefix + n_base2 -
           task.common_prefix;
      for (size_t j = n_base2 - 1; j < task.seq_tokens[1].size() - 1 - last_2nd;
           ++j) {
        eval_pairs.emplace_back(task.i_logits + li++,
                                task.seq_tokens[1][j + 1]);
      }
    }
    compute_logprobs(batch_logits.data(), n_vocab, workers, eval_pairs,
                     eval_results);
    ...
    for (size_t i = i0; i < i1; ++i) {
      ...
      printf("%zu\t%.4lf\t%10.6f  %10.6f  %d  %d\n", i + 1,
             100.0 * n_correct / n_done, score_1st, score_2nd, result,
             task.answer);
      fflush(stdout);
    }
...
static bool deserialize_string(std::istream &in, std::string &str) {
  ...
  if (!in.read((char *)&size, sizeof(size)).fail()) {
    str.resize(size);
    if (!in.read((char *)&str[0], size).fail())
      return true;
  }
  return false;
}
...
struct multiple_choice_answers {
  ...
  bool deserialize(std::istream &in) {
    ...
    if (in.fail() || n > 100)
      return false; // 100 as max. number of answers should be good enough for
                    // any practical purpose
    answers.resize(n);
    labels.resize(n);
    for (auto &a : answers) {
      if (!deserialize_string(in, a))
        return false;
    }
...
struct multiple_choice_task {
  ...
  multiple_choice_answers
      mc1; // possible answers (continuations) with a single correct answer
  multiple_choice_answers mc2; // possible answers (continuations) with multiple
                               // correct answers - not handled yet
  bool deserialize(std::istream &in) {
    if (!deserialize_string(in, question))
      return false;
  ...
  // For evaluation
  size_t i_logits;      // starting index of logits in the llama_batch
  size_t common_prefix; // max number of initial tokens that are the same in all
                        // sentences
...
static bool multiple_choice_prepare_one_task(llama_context *ctx,
  ...
  for (auto &answer : task.mc1.answers) {
    ...
    task.seq_tokens.emplace_back(
        ::llama_tokenize(ctx, task.question + " " + answer, true));
  }
...
//
// Calculates score for multiple choice tasks with single correct answer from
// prompt. Commonly used LLM evaluation metrics of this type are
//   * ARC
//   * HellaSwag
//   * MMLU
//   * TruthfulQA
...
//
static void multiple_choice_score(llama_context *ctx,
                                  const gpt_params &params) {
  ...
  if (params.multiple_choice_tasks == 0 ||
      params.multiple_choice_tasks >= (size_t)n_task) {
    // Use all tasks
    tasks.resize(n_task);
    ...
    printf("%s: selecting %zu random tasks from %u tasks available\n", __func__,
           params.multiple_choice_tasks, n_task);
    std::mt19937 rng(1);
    ...
    auto prepare = [&counter, &n_bad, &tasks, ctx]() {
      ...
      while (true) {
        ...
        for (int i = first; i < last; ++i) {
          if (!multiple_choice_prepare_one_task(ctx, tasks[i], false))
            ++n_bad_local;
        }
      }
    };
    size_t max_thread = std::thread::hardware_concurrency();
    max_thread = std::min(max_thread,
                          (tasks.size() + K_TOKEN_CHUNK - 1) / K_TOKEN_CHUNK);
    std::vector<std::thread> workers(max_thread - 1);
    for (auto &w : workers)
      w = std::thread(prepare);
    prepare();
    for (auto &w : workers)
      w.join();
    printf("done\n");
  ...
  for (size_t i0 = 0; i0 < tasks.size(); i0++) {
    ...
    while (n_cur + (int)tasks[i1].required_tokens <= n_ctx) {
      ...
      for (size_t i = 0; i < cur_task.common_prefix; ++i) {
        // llama_batch_add(batch, cur_task.seq_tokens[0][i], i, { s0 + 0, s0 +
        // 1, s0 + 2, s0 + 3}, false);
        llama_batch_add(batch, cur_task.seq_tokens[0][i], i, batch_indeces,
                        false);
      }
      batch.logits[batch.n_tokens - 1] =
          true; // we need logits for the last token of the common prefix
      ...
      for (int s = 0; s < int(cur_task.seq_tokens.size()); ++s) {
        ...
        for (size_t i = cur_task.common_prefix; i < seq_tokens_size; ++i) {
          const bool needs_logits = i < seq_tokens_size - 1;
          llama_batch_add(batch, cur_task.seq_tokens[s][i], i, {s0 + s},
                          needs_logits);
          n_logits += needs_logits;
        }
      }
    ...
    for (size_t i = i0; i < i1; ++i) {
      ...
      size_t li = 1; // skip the last logit of the common prefix (computed
                     // separately below)
      for (int s = 0; s < int(cur_task.seq_tokens.size()); ++s) {
        for (size_t j = cur_task.common_prefix;
             j < cur_task.seq_tokens[s].size() - 1; j++) {
          eval_pairs.emplace_back(cur_task.i_logits + li++,
                                  cur_task.seq_tokens[s][j + 1]);
        }
      }
    }
    // Then we do the actual calculation
    ...
    for (size_t i = i0; i < i1; ++i) {
      auto &cur_task = tasks[i];
      // printf("==== Evaluating <%s> with correct answer ",
      // cur_task.question.c_str()); for (int j = 0; j <
      // int(cur_task.mc1.labels.size()); ++j) {
      ...
      // get the logits of the last token of the common prefix
      std::memcpy(tok_logits.data(),
                  batch_logits.data() + n_vocab * cur_task.i_logits,
                  n_vocab * sizeof(float));
      ...
      for (int s = 0; s < int(cur_task.seq_tokens.size()); ++s) {
        size_t count = 1;
        float log_prob = std::log(
            first_probs[cur_task.seq_tokens[s][cur_task.common_prefix]]);
        for (size_t j = cur_task.common_prefix;
             j < cur_task.seq_tokens[s].size() - 1; j++) {
          // printf("        %zu  %g\n", ir, eval_results[ir]);
          ++count;
  ...
  if (n_done < 100 && (params.multiple_choice_tasks != 0 &&
                       params.multiple_choice_tasks < (size_t)n_task))
    return;
...
static void kl_divergence(llama_context *ctx, const gpt_params &params) {
  if (params.logits_file.empty()) {
    fprintf(stderr,
            "%s: you must provide a name of a file containing the log "
            "probabilities of the base model\n",
            __func__);
    return;
  }
  ...
  }
  {
    ...
    if (in.fail() || strncmp("_logits_", check, 8) != 0) {
      fprintf(stderr,
              "%s: %s does not look like a file containing log-probabilities\n",
  ...
  if (n_ctx > llama_n_ctx(ctx)) {
    fprintf(stderr,
            "%s: %s has been computed with %u, while the current context is "
            "%d. Increase it with -c and retry\n",
  ...
  if (in.read((char *)tokens.data(), tokens.size() * sizeof(tokens[0]))
          .fail()) {
  ...
  for (int i = 0; i < n_chunk; ++i) {
    ...
    if (in.read((char *)log_probs_uint16.data(),
                log_probs_uint16.size() * sizeof(uint16_t))
            .fail()) {
    ...
    const int first = n_ctx / 2;
    const float *all_logits =
        num_batches > 1 ? logits.data() : llama_get_logits(ctx);
    process_logits(n_vocab, all_logits + first * n_vocab,
                   tokens.data() + start + first, n_ctx - 1 - first, workers,
                   log_probs_uint16, kld, kld_ptr, p_diff_ptr);
    ...
    const double ppl_unc =
        ppl_val * log_ppl.second; // ppl_unc = sqrt( (dexp(x) / dx) ** 2 *
                                  // log_ppl.second ** 2 )
    printf("    %9.4lf ± %9.4lf", ppl_val, ppl_unc);

    auto log_ppl_base =
        mean_and_uncertainty(kld.sum_nll_base, kld.sum_nll_base2, kld.count);
    const double log_ppl_cov = covariance(kld.sum_nll, kld.sum_nll_base,
                                          kld.sum_nll_nll_base, kld.count);
    const double log_ppl_ratio_val = log_ppl.first - log_ppl_base.first;
    const double log_ppl_ratio_unc =
        sqrt(log_ppl.second * log_ppl.second +
    ...
    auto p_diff_mse =
        mean_and_uncertainty(kld.sum_p_diff2, kld.sum_p_diff4, kld.count);
    const double p_diff_rms_val = sqrt(p_diff_mse.first);
    const double p_diff_rms_unc = 0.5 / p_diff_rms_val * p_diff_mse.second;
    printf("    %6.3lf ± %6.3lf %%", 100.0 * p_diff_rms_val,
           100.0 * p_diff_rms_unc);
  ...
  if (kld.count < 100)
    return; // we do not wish to do statistics on so few values
  ...
  const double ppl_base_unc =
      ppl_base_val * log_ppl_base.second; // ppl_base_unc = sqrt( (dexp(x) / dx)
                                          // ** 2 * log_ppl_base.second ** 2 )
  ...
  const double log_ppl_cor =
      log_ppl_cov / (log_ppl.second * log_ppl_base.second);
  ...
  const double ppl_ratio_unc =
      ppl_ratio_val * log_ppl_ratio_unc; // ppl_ratio_unc = sqrt( (dexp(x) / dx)
                                         // ** 2 * log_ppl_ratio.second ** 2 )
  ...
  const double ppl_diff_unc =
      sqrt(ppl_unc * ppl_unc + ppl_base_unc * ppl_base_unc - 2.0 * ppl_cov);
  printf("Mean PPL(Q)-PPL(base)         : %10.6lf ± %10.6lf\n", ppl_diff_val,
         ppl_diff_unc);
  ...
  auto kld_median = kld_values.size() % 2 == 0
                        ? 0.5f * (kld_values[kld_values.size() / 2] +
                                  kld_values[kld_values.size() / 2 - 1])
  ...
  auto percentile = [](std::vector<float> values, float fraction) {
    ...
    size_t ip = size_t(p);
    p -= ip;
    return (1 - p) * values[ip] +
           p * values[std::min(ip + 1, values.size() - 1)];
  };
  ...
  auto p_diff =
      mean_and_uncertainty(kld.sum_p_diff, kld.sum_p_diff2, kld.count);
  printf("Mean    Δp: %6.3lf ± %5.3lf %%\n", 100.0 * p_diff.first,
         100.0 * p_diff.second);

  auto p_diff_median =
      p_diff_values.size() % 2 == 0
          ? 0.5f * (p_diff_values[p_diff_values.size() / 2] +
                    p_diff_values[p_diff_values.size() / 2 - 1])
...
int main(int argc, char **argv) {
  ...
  if (params.ppl_stride > 0) {
    fprintf(stderr,
            "Will perform strided perplexity calculation -> adjusting context "
            "size from %d to %d\n",
  ...
  init_parameters(params.n_ctx, params.prompt_size, params.k_encode_size,
                  params.v_encode_size, params.use_encode, params.cache_mmap);
