...
static const ggml_type_traits_t type_traits[GGML_TYPE_COUNT] = {
    ...
    [GGML_TYPE_BF16] = {
        ...
        .type_name                = "q8_roy",
        .blck_size                = QK8_ROY,
        .type_size                = sizeof(block_q8_roy),
        .is_quantized             = true,
        .from_float               = quantize_row_q8_roy,
        .from_float_reference     = (ggml_from_float_t) quantize_row_q8_roy_reference,
        .vec_dot_type             = GGML_TYPE_Q8_ROY,
        .nrows                    = 1,
    },
        [GGML_TYPE_Q4_V_ROY] = {
...
static void ggml_compute_forward_dup_f32(
    ...
    if (ggml_is_contiguous(dst)) {
        ...
        if (nb00 == sizeof(float)) {
            ...
            } else if (type_traits[dst->type].from_float || dst->type == GGML_TYPE_Q4_ROY) {
                ggml_from_float_t const quantize_row_q = type_traits[dst->type].from_float;
                    ...
                    for (int i02 = 0; i02 < ne02; i02++) {
                        ...
                        for (int i01 = ir0; i01 < ir1; i01++) {
                            ...
                            if(dst->type == GGML_TYPE_Q4_ROY){
                              quantize_row_q4_roy_reference(src0_ptr, ne00, i01, layer_id);
                            }else{
...
static void ggml_compute_forward_mul_mat_one_chunk(
    ...
    if(src0->type == GGML_TYPE_Q4_ROY){
      ...
      if(dst->name[5] == '\0'){
        layer_id = dst->name[4]-'0';
      }else{
        layer_id = 10 * (dst->name[4]-'0') + dst->name[5]-'0';
      }
    }
        ...
        for (int64_t iir0 = ir0_start; iir0 < ir0_end; iir0 += blck_0) {
            for (int64_t ir1 = iir1; ir1 < iir1 + blck_1 && ir1 < ir1_end; ir1 += num_rows_per_vec_dot) {
                ...
                for (int64_t ir0 = iir0; ir0 < iir0 + blck_0 && ir0 < ir0_end; ir0 += num_rows_per_vec_dot) {
                    if(src0->type == GGML_TYPE_Q4_ROY){
                      ggml_vec_dot_q4_roy_q8_roy(ne00, &tmp[ir0 - iir0], src1_col, ir0, i02, layer_id);
...
static void ggml_compute_forward_mul_mat(
    ...
    if (src1_cont && src0->type!=GGML_TYPE_Q4_V_ROY) {
        for (int64_t i13 = 0; i13 < ne13; i13++)
            for (int64_t i12 = 0; i12 < ne12; i12++)
...
#if GGML_USE_LLAMAFILE
    if (src1->type != vec_dot_type && src0->type!=GGML_TYPE_Q4_V_ROY) {
        const void* wdata = (src1->type == vec_dot_type) ? src1->data : params->wdata;
        const size_t row_size = ggml_row_size(vec_dot_type, ne10);
