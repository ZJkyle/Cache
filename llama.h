...
struct llama_model_params {
  ...
  // LLAMA_SPLIT_ROW: the GPU that is used for small tensors and intermediate
  // results LLAMA_SPLIT_LAYER: ignored
  int32_t main_gpu;
...
struct llama_context_params {
  ...
  // Keep the booleans together to avoid misalignment during copy-by-value.
  bool logits_all;  // the llama_decode() call computes all logits, not just the
                    // last one (DEPRECATED - set llama_batch.logits instead)
...
typedef struct llama_model_quantize_params {
  int32_t nthread; // number of threads to use for quantizing, if <=0 will use
...
LLAMA_API enum llama_pooling_type
llama_pooling_type(const struct llama_context *ctx);
...
LLAMA_API int32_t llama_model_apply_lora_from_file(
    const struct llama_model *model, const char *path_lora, float scale,
    const char *path_base_model, int32_t n_threads);
...
LLAMA_API int32_t llama_control_vector_apply(struct llama_context *lctx,
...
llama_kv_cache_view_init(const struct llama_context *ctx, int32_t n_seq_max);
...
LLAMA_API
DEPRECATED(size_t llama_get_state_size(const struct llama_context *ctx),
...
LLAMA_API size_t llama_state_seq_get_data(struct llama_context *ctx,
                                          uint8_t *dst, llama_seq_id seq_id);
...
// EOT, etc.)
LLAMA_API bool llama_token_is_eog(const struct llama_model *model,
                                  llama_token token);
...
LLAMA_API llama_token
llama_token_cls(const struct llama_model *model); // classification
LLAMA_API llama_token
llama_token_sep(const struct llama_model *model); // sentence separator
LLAMA_API llama_token
...
LLAMA_API int32_t llama_token_to_piece(const struct llama_model *model,
                                       llama_token token, char *buf,
                                       int32_t length, bool special);
...
/// template.
LLAMA_API int32_t
llama_chat_apply_template(const struct llama_model *model, const char *tmpl,
...
/// @param logits_guidance Logits extracted from a separate context from the
/// same model. Other than a negative prompt at the beginning, it should have
/// all generated and user input tokens copied from the main context.
/// @param scale Guidance strength. 1.0f means no guidance. Higher values mean
/// stronger guidance.
LLAMA_API void llama_sample_apply_guidance(struct llama_context *ctx,
                                           float *logits,
                                           float *logits_guidance, float scale);
...
/// @details Dynamic temperature implementation described in the paper
/// https://arxiv.org/abs/2309.02772.
LLAMA_API void llama_sample_entropy(struct llama_context *ctx,
...
/// probabilities using the RNG of ctx.
...
LLAMA_API void llama_beam_search(struct llama_context *ctx,
                                 ...
                                 int32_t n_past, int32_t n_predict);
...
decode_utf8(const std::string &src, llama_partial_utf8 partial_start);

// Randomly selects a token from the candidates based on their probabilities
// using given std::mt19937. This is a temporary workaround in order to fix race
// conditions when sampling with multiple sequences.
llama_token llama_sample_token_with_rng(struct llama_context *ctx,
                                        llama_token_data_array *candidates,
                                        std::mt19937 &rng);
